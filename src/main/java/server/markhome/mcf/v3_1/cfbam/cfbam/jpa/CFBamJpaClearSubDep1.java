...
@Table(
	...
	indexes = {
		...
		@Index(name = "ClearSubDep1ClearTopDepIdxClearTopDep", columnList = "contclrdepidClearTopDep", unique = false)
	}
)
	...
	implements ICFBamClearSubDep1
{
	...
	@JoinColumn( name="contclrdepidClearTopDep", referencedColumnName="Id" )
	...
	public void setRequiredContainerClearTopDep(ICFBamClearTopDep argObj) {
		if(argObj == null) {
			...
			if (requiredContainerClearTopDep != null) {
				requiredClearTopDepId = requiredContainerClearTopDep.getRequiredId();
			}
			else {
			}
		}
	...
	public void setRequiredContainerClearTopDep(CFLibDbKeyHash256 argClearTopDepId) {
		...
		ICFBamClearTopDep targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argClearTopDepId);
		setRequiredContainerClearTopDep(targetRec);
	}

	@Override
	public List<ICFBamClearSubDep2> getOptionalComponentsClearDep() {
		List<ICFBamClearSubDep2> retlist = (optionalComponentsClearDep != null) ? new ArrayList<>(optionalComponentsClearDep) : new ArrayList<>();
		return( retlist );
	}
