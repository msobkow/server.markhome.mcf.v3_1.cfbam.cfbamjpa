...
@Table(
	...
	indexes = {
		...
		@Index(name = "DelSubDep1DelTopDepIdxDelTopDep", columnList = "contdeldepidDelTopDep", unique = false)
	}
)
	...
	implements ICFBamDelSubDep1
{
	...
	public void setRequiredContainerDelTopDep(ICFLibKeyHash256 argDelTopDepId) {
		...
		ICFBamDelTopDep targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argDelTopDepId);
		setRequiredContainerDelTopDep(targetRec);
	}

	@Override
	public List<ICFBamDelSubDep2> getOptionalComponentsDelDep() {
		List<ICFBamDelSubDep2> retlist = (optionalComponentsDelDep != null) ? new ArrayList<>(optionalComponentsDelDep) : new ArrayList<>();
		return( retlist );
	}
