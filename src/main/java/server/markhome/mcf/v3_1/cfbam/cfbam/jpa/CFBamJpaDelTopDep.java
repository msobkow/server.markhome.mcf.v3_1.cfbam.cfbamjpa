	...
	implements ICFBamDelTopDep
{
	...
	public void setOptionalLookupPrev(ICFLibKeyHash256 argPrevId) {
		...
		ICFBamDelTopDep targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argPrevId);
	...
	public void setOptionalLookupNext(ICFLibKeyHash256 argNextId) {
		...
		ICFBamDelTopDep targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argNextId);
		setOptionalLookupNext(targetRec);
	}

	@Override
	public List<ICFBamDelSubDep1> getOptionalComponentsDelDep() {
		List<ICFBamDelSubDep1> retlist = (optionalComponentsDelDep != null) ? new ArrayList<>(optionalComponentsDelDep) : new ArrayList<>();
		return( retlist );
	}
