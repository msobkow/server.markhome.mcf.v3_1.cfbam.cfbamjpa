...
@Table(
	...
	indexes = {
		...
		@Index(name = "PopSubDep1PopTopDepIdxContPopTopDep", columnList = "contpopdepidContPopTopDep", unique = false)
	}
)
	...
	implements ICFBamPopSubDep1
{
	...
	@JoinColumn( name="contpopdepidContPopTopDep", referencedColumnName="Id" )
	...
	public void setRequiredContainerContPopTopDep(ICFBamPopTopDep argObj) {
		if(argObj == null) {
			...
			if (requiredContainerContPopTopDep != null) {
				requiredPopTopDepId = requiredContainerContPopTopDep.getRequiredId();
			}
			else {
			}
		}
	...
	public List<ICFBamPopSubDep2> getOptionalComponentsPopDep() {
		List<ICFBamPopSubDep2> retlist = (optionalComponentsPopDep != null) ? new ArrayList<>(optionalComponentsPopDep) : new ArrayList<>();
		return( retlist );
	}
