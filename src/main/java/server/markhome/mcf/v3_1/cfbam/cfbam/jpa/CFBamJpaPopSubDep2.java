	...
	implements ICFBamPopSubDep2
{
	...
	public void setRequiredContainerPopSubDep1(ICFBamPopSubDep1 argObj) {
		if(argObj == null) {
			...
			if (requiredContainerPopSubDep1 != null) {
				requiredPopSubDep1Id = requiredContainerPopSubDep1.getRequiredId();
			}
			else {
			}
		}
	...
	public void setRequiredContainerPopSubDep1(CFLibDbKeyHash256 argPopSubDep1Id) {
		...
		ICFBamPopSubDep1 targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argPopSubDep1Id);
		setRequiredContainerPopSubDep1(targetRec);
	}

	@Override
	public List<ICFBamPopSubDep3> getOptionalComponentsPopDep() {
		List<ICFBamPopSubDep3> retlist = (optionalComponentsPopDep != null) ? new ArrayList<>(optionalComponentsPopDep) : new ArrayList<>();
		return( retlist );
	}
