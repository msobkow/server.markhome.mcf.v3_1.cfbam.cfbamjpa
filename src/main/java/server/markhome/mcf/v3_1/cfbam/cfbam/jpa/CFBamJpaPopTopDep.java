	...
	implements ICFBamPopTopDep
{
	...
	public void setRequiredContainerContRelation(CFLibDbKeyHash256 argContRelationId) {
		...
		ICFBamRelation targetRec = targetTable.readDerived(ICFSecSchema.getAuthorizationCallback().getEffectiveAuthorization(), argContRelationId);
		setRequiredContainerContRelation(targetRec);
	}

	@Override
	public List<ICFBamPopSubDep1> getOptionalComponentsPopDep() {
		List<ICFBamPopSubDep1> retlist = (optionalComponentsPopDep != null) ? new ArrayList<>(optionalComponentsPopDep) : new ArrayList<>();
		return( retlist );
	}
